...
function createMarkdownComponents(options: MarkdownComponentOptions = {}): Components {
  const bodyClass = "m-0 mb-4 last:mb-0 leading-[1.65] text-zinc-300";
  ...
  return {
    ...
    ul: ({ children }) => (
      <ul className="m-0 mb-4 list-disc space-y-1.5 pl-5 leading-[1.6] text-zinc-300 last:mb-0">
        {children}
      </ul>
    ),
    ol: ({ children }) => (
      <ol className="m-0 mb-4 list-decimal space-y-1.5 pl-5 leading-[1.6] text-zinc-300 last:mb-0">
        {children}
      </ol>
    ),
    li: ({ children }) => <li className="pl-0.5">{children}</li>,
    blockquote: ({ children }) => (
      <blockquote className="m-0 mb-4 border-l-2 border-white/15 pl-3.5 text-zinc-400 italic leading-[1.6] last:mb-0">
        {children}
      </blockquote>
    ),
    strong: ({ children }) => {
      ...
      return (
        <button
          ...
          className="font-semibold text-zinc-100 underline decoration-white/25 underline-offset-2 can-hover:hover:decoration-white/60 cursor-pointer bg-transparent border-0 p-0 inline text-inherit align-baseline"
        >
          {children}
        </button>
      );
    ...
    h1: ({ children }) => (
      <p className="m-0 mb-4 text-[15px] font-semibold leading-snug text-zinc-100">{children}</p>
    ),
    h2: ({ children }) => (
      <p className="m-0 mb-3 mt-1 text-[14px] font-semibold leading-snug text-zinc-100">{children}</p>
    ),
    h3: ({ children }) => (
      <p className="m-0 mb-2.5 mt-1 text-[14px] font-medium leading-snug text-zinc-200">{children}</p>
    ),
    img: ({ src, alt }) =>
      isBannerImage(src ?? "") ? (
    ...
    code: ({ className, children }) => {
      if (className) {
        return (
          <pre className="mb-4 overflow-auto rounded-md bg-black/30 p-3 text-[13px] leading-relaxed last:mb-0">
            <code>{children}</code>
          </pre>
        );
      }
      return (
        <code className="rounded bg-white/8 px-1 py-0.5 text-[13px] text-zinc-200">{children}</code>
      );
    },
  };
}
...
export function IntroReadmeContent({
  ...
  if (leading) {
    if (isBannerImage(leading.src)) {
      return (
        <div className="readme-markdown text-[14px] text-zinc-300">
